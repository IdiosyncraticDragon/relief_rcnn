  ...
  if (blob_name_to_idx && layer_param && layer_param->bottom_size() > top_id &&
    ...
    if (layer_id == -1) {
      ...
      if (param.input_dim_size() > 0) {
        ...
        //------added by LGY @2016.4.15
        BlobShape tmp_shape = param.input_shape(top_id);
        LOG(INFO) << "Input " << top_id << " -> hello ^_^";
        for (int i=0; i < tmp_shape.dim_size(); ++i) {
             LOG(INFO) << "dim: " << tmp_shape.dim(i);
        }
        //------added by LGY @2016.4.15
...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  //LOG(INFO)<<"Hi, Forwarding! from"<< start << " to "<<end << " debug: "<<debug_info_;
  Dtype loss = 0;
  if (debug_info_) {
