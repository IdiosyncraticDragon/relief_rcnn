...
def demo_iouBoundary_rilievo(net, net_proposal, image_name, classes, savename,loop_counts=1):
    ...
    def map_back_from_pool1(indices, layer_param=None):
         ...
         if layer_param != None:
            ...
            if y_max>26:
               y_max=26
         x_min=x_min*pool_stride-1
         x_max=x_max*pool_stride+pool_size-1-1
         y_min=y_min*pool_stride-1
         y_max=y_max*pool_stride+pool_size-1-1
         x_min=x_min*conv_stride-5
         x_max=x_max*conv_stride+conv_size-1-5
         y_min=y_min*conv_stride-5
         y_max=y_max*conv_stride+conv_size-1-5
    ...
    def map_back_small_part_from_pool1(indices,fmap,layer_param=None):
         ...
         while candidate_count>0:
            ...
            candidate_count=len(candidates)
         xx_boxes=generate_clusters(indices)
         aa=np.zeros([xx_boxes.shape[0],1])
         xx_boxes=np.hstack((xx_boxes,aa))
         new_boxes=np.vstack((new_boxes,xx_boxes))
         x_min=new_boxes[:,0]
         x_max=new_boxes[:,2]
         y_min=new_boxes[:,1]
         y_max=new_boxes[:,3]
    ...
    input_proposals=np.zeros([0,4])
    levelmap=np.zeros([76,101])
    for fi in range(1):
               level_num=10
               f_proposals=np.zeros([0,4])
               fmap=np.zeros([76,101])
               temp_proposals=np.zeros([0,5])
               for fj in range(net.blobs['pool1'].data.shape[1]):
                  f1=net.blobs['pool1'].data[fi,fj,:,:]
                  if np.max(f1)==0:
                     continue
                  f1=np.divide(f1,np.max(f1))
                  fmap= np.add(fmap,f1)
               ...
               if selected_indices[0].shape[0]!=0:
                  ...
                  selected_indices=map_back_from_fm(selected_indices, 1, 1, fmap)
                  temp_proposals = np.vstack((temp_proposals,selected_indices))
               temp_proposals = box_backtrace(temp_proposals[:,0:-1], im_width/600, im_height/800)
               input_proposals = np.vstack((input_proposals,temp_proposals))
    #BOXSIZE_THRESH=2500
...
if __name__ == '__main__':
    ...
    with open('voc_val.txt','r') as fd:
         i=1
         #for line in fd:
         for line in ['002940','009647','009443','009113','009087','008424','007786','006029','005653','005326','004585']:
             fname=line.strip()
             #fname='005653'
             #fname='000268'
             fname='009113'
